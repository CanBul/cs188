...
def cornersHeuristic(state, problem):
    ...
    for i, corner in enumerate(corners):
        ...
        dist_x = abs(corner_x - loc_x)
        dist_y = abs(corner_y - loc_y)
        total_dist = dist_x + dist_y

        distance = max(distance, total_dist)
...
def foodHeuristic(state, problem):
    ...
    BooleanFoodState = [i for group in foodGrid for i in group]
    # Goal State
    if False not in BooleanFoodState:
        return 0

    # Find the furthest two foods and get add the distance of pacman to the closest one
    foodList = foodGrid.asList()

    # remainingFoodCoordinates = []

    # for i, each in enumerate(BooleanFoodState):
    #     if each:
    #         remainingFoodCoordinates.append(foodList[i])

    node1, node2, distance = findFurthestPoints(foodList)

    if len(foodList) == 0:
        return 0

    distanceToNode1 = ManDistance(position, node1)
    distanceToNode2 = ManDistance(position, node2)

    pacmanToSmallNode = min(distanceToNode1, distanceToNode2)

    return distance + pacmanToSmallNode

    # for i, food in enumerate(foodGrid.asList()):
    #     if not BooleanFoodState[i]:
    #         continue
    #     food_x, food_y = food
    #     distance = abs(food_x-pac_x) + abs(pac_y-food_y)
    #     if distance >= distance_to_pacman:
    #         distance_to_pacman = distance
    #         firstFoodCoordinate = (food_x, food_y)
